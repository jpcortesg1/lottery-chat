...
class MyAppState extends ChangeNotifier {
  ...
  void createUserData() async {
    try {
      final url =
          Uri.parse('http://192.168.0.22:8000/api/v1/users/create-update');
  ...
  void validateUserData() async {
    try {
      final url =
          Uri.parse('http://192.168.0.22:8000/api/v1/users?document=$document');
      final response = await http.get(url);
      ...
      if (baseData['data'].isNotEmpty) {
        final userData = baseData['data'][0];
        name = userData['name'];
        email = userData['email'];
        cellphone = userData['cellphone'];
        messages.add(Message.create(
            'Datos encontrados: Nombre: $name, Correo: $email, Celular: $cellphone',
            false));
        messages.add(
